...
class DecisionTree:
    ...
    def learn_tree(self, examples):
        ...
        #
        initial = examples[0].get(self.class_name)
        allsame = True
        for e in examples:
            if e.get(self.class_name) != initial:
                allsame = False
                break
        if allsame == True:
            return LeafNode(initial, len(examples), len(examples))
        if len(examples) < 2*self.min_leaf_count:
            return self.createleaf(examples)
        bestval = 0.0
        bestlab = ""
        bestgt = []
        bestlt = []
        bestmiss = []
        bestig = -math.inf
        for feat in examples[0].keys():
            if feat != self.id_name and feat != self.class_name:
                values = []
                for e in examples:
                    if not e.get(feat) is None:
                        values.append(e.get(feat))
                v = set(values)
                for val in v:
                    currgt = []
                    currlt = []
                    ...
                    currig = 0.0
                    for ex in examples:
                        if ex.get(feat) is None:
                            currmiss.append(ex)
                        elif ex.get(feat) < val:
                            currlt.append(ex)
                        else:
                            currgt.append(ex)
                    currig = self.infogain(examples, currlt, currgt)
                    if currig > bestig and len(currgt) >= self.min_leaf_count and len(currlt) >= self.min_leaf_count:
                        bestval = val
                        bestlab = feat
                        bestgt = currgt
                        bestlt = currlt
                        bestmiss = currmiss
                        bestig = currig
        if len(bestlt) < self.min_leaf_count or len(bestgt) < self.min_leaf_count:
            return self.createleaf(examples)
        if len(bestlt) > len(bestgt):
            return DecisionNode(bestlab, bestval, self.learn_tree(bestlt), self.learn_tree(bestgt), self.learn_tree(bestlt))
        return DecisionNode(bestlab, bestval, self.learn_tree(bestlt), self.learn_tree(bestgt), self.learn_tree(bestgt))  # fix this line!

    def infogain(self, parent, lessthan, greatthan):
        return float(self.entropy(parent)) - (self.entropy(lessthan) * (float(len(lessthan))/len(parent))) - (self.entropy(greatthan) * (float(len(greatthan))/len(parent)))

    def createleaf(self, examples):
        labels = {}
        for e in examples:
            if e.get(self.class_name) in labels:
                labels[e.get(self.class_name)] += 1
            else:
                labels[e.get(self.class_name)] = 1
        largestval = 0
        largestlab = ""
        for k, v in labels.items():
            if v > largestval:
                largestval = v
                largestlab = k
        return LeafNode(largestlab, largestval, len(examples))
    ...
    def entropy(self, examples):
        ...
        entropy = 0.0
        for k, v in labels.items():
            prob = float(v)/len(examples)
            entropy += (float(prob)*math.log2(prob))
        return -1 * float(entropy)
